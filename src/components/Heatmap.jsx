...
export default function Heatmap({ plan, completions }) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px', overflow: 'hidden' }}>
      <div style={{ fontFamily: 'Exo 2, sans-serif', fontSize: 10, color: 'var(--muted)', letterSpacing: 3, marginBottom: 10 }}>TRAINING LOAD · 25 WEEKS</div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 10, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontFamily: 'Exo 2, sans-serif', fontSize: 10, color: 'var(--muted)', letterSpacing: 1 }}>{label.toUpperCase()}</span>
          </div>
        ))}
      </div>
    </div>
